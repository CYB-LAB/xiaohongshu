    ...
    public void noteCollectCountShardingJobHandler() throws Exception {
        ...
        for (; ; ){
            ...
            noteIds.forEach(noteId -> {
                // 2: 对 t_note_collection 笔记收藏表执行 count(*) 操作，获取收藏总数
                int noteCollectTotal = selectMapper.selectNoteCollectCountFromNoteCollectionTableByNoteId(noteId);

                // 3: 更新 t_note_count 表， 更新对应 Redis 缓存
                int count = updateMapper.updateNoteCollectTotalByNoteId(noteId, noteCollectTotal);
                ...
                if (count > 0) {
                    ...
                    if (hashKey) {
                        ...
                        redisTemplate.opsForHash().put(redisKey, RedisKeyConstants.FIELD_COLLECT_TOTAL, noteCollectTotal);
                    }
                }
            });
