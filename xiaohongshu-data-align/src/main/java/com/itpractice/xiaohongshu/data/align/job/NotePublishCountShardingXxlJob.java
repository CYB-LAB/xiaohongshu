    ...
    public void notePublishCountShardingJobHandler() throws Exception {
        ...
        for (; ; ) {
            ...
            userIds.forEach(userId -> {
                // 2: 对 t_note 笔记表执行 count(*) 操作，获取笔记发布总数
                int notePublishTotal = selectMapper.selectCountFromNoteTableByUserId(userId);

                // 3: 更新 t_user_count 表, 添加对应 Redis 缓存
                int count = updateMapper.updateNotePublishTotalByNoteId(userId, notePublishTotal);
