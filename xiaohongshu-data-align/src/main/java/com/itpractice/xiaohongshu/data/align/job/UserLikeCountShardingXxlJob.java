    ...
    public void userLikeCountShardingJobHandler() throws Exception {
        ...
        for (; ; ){
            ...
            userIds.forEach(userId -> {
                // 2: 对 t_note_like 笔记点赞表执行 count(*) 操作，获取点赞总数
                int userLikeTotal = selectMapper.selectUserCountFromNoteLikeTableByUserId(userId);

                // 3: 更新 t_user_count 表, 添加对应 Redis 缓存
        ...
        XxlJobHelper.log("=================> 结束定时分片广播任务：对当日发生变更的用户获得点赞数进行对齐，共对齐记录数：{}", processedTotal);
    }
}
