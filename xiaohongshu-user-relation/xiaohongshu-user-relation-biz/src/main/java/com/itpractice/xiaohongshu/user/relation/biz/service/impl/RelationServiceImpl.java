package com.itpractice.xiaohongshu.user.relation.biz.service.impl;
...
import com.itpractice.xiaohongshu.user.relation.biz.domain.dataobject.FansDO;
import com.itpractice.xiaohongshu.user.relation.biz.domain.dataobject.FollowingDO;
        ...
        if (total > 0) { // 缓存中有数据
            ...
            if (CollUtil.isNotEmpty(fansDOS)) {
                ...
                List<Long> userIds = fansDOS.stream()
                        .map(FansDO::getFansUserId) // 此时操作的都是非 null 的 FansDO，安全
                        .toList();
